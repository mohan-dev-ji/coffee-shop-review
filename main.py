...
class Comment(db.Model):
    ...
    post_id: Mapped[str] = mapped_column(Integer, db.ForeignKey("cafe.id"))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Email doesn't exist
        if not user:
            flash("That email does not exist, please register if you are new.")
            return redirect(url_for('login'))
        # Password incorrect
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        ...
        new_comment = Comment(
            ...
            rating=comment_form.star_rating.data,
